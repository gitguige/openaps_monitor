...
def gen_code_common_multiplestoptime(title,fileLoc,faultLoc,variable,newvalue,newvalue2=0,early_start=False):
	...
	if early_start:
		...
		time_partition_start =  np.arange(6,30,2).tolist() + np.arange(30,80,10).tolist()
	else:
	...
	for valueitem in valuelist:
		for i in range(17): #
			trigger_time = time_partition_start[i]
			duration = 30
			# for i in np.arange(trigger_time,120,duration):
				# if i+duration <= 121: #make sure the stop time is no more than 121
			stop_time = trigger_time+duration #hong long fault will last, at least one iteration
