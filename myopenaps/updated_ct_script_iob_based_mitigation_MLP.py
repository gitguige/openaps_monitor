...
if Mitigation_Enable:
  ...
  new_model = tf.keras.models.load_model('../Reseult/script_jupyternotebook/saved_model/MLP_model_{}'.format(threshold_col))
######################################################
...
# Fault_injection ############# permanent hardware fault injection #################################  
  ...
  with open('monitor/glucose.json', 'w') as outfile:f_bg = 1.509*(math.pow((math.log(float(data["CGM_glucose"][i]))), 1.084) - 5.381)
...
#########################=============inject fault here==============#####################
  ...
  if Mitigation_Enable:
    ...
    f_bg = 1.509*(math.pow((math.log(float(bg))), 1.084) - 5.381)
    ...
    average_window = min(len(rl_bg),12) #take average over 1 hour(=12*5minutes) time windows after 12th frame
    LBGI=(sum(rl_bg[-1*average_window:])/average_window)
    HBGI=(sum(rh_bg[-1*average_window:])/average_window)
    BGI = HBGI+ LBGI

    if BGI - BGI_t2>0.045 and (LBGI>5 or HBGI>9):
      hazard_flag =1
    else:
      hazard_flag =0

    X_test =np.array([bg, iob,loaded_suggested_data["rate"],LBGI,\
                               HBGI,BGI,hazard_flag,del_bg,del_iob,del_rate]).reshape(1,10)

    sub_alert_flag=np.argmax(new_model.predict(X_test),axis=1)
