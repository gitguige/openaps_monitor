...
for file in glob.glob("data_*.csv"):
		...
		if bg < bgLowerTh:
			...
			# checking if BG is rising
				...
				if delIob > 0: # IOB is rising
					if delInsulinRate == 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_21" # New context table
				elif delIob < 0: # IOB is falling
					if delInsulinRate == 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_22" # New context table
				...
				if delIob >=0: # IOB is not falling
					if delInsulinRate == 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_23" # New context table
				...
				if delIob < 0:
					if delInsulinRate == 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_25" # New context table
				if delIob > 0:
					if delInsulinRate == 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_24" # New context table
			...
			if delBg > 0:
				if delIob <=0: # IOB is not rising
					if delInsulinRate == 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_26" # New context table
				...
				if delIob <=0: # IOB is not rising
					if delInsulinRate == 0:
						data["detection"][i] = 100
						data["unsafe_action_reason"][i] = "row_27" # New context table
				# checking if BG is falling more than the threshold
				#if delBg < thBgFall:
	...
	writeNumDetection = open("output/result/WT2nd_KI_"+file_name+".txt", "w")
	writeNumDetection.write("Number of Alarm: "+str(numDetection)+"\n")
	writeNumDetection.close()

	## Add Alarm Time
	appendAlamrTime = open("output/result/WT2nd_KI_"+file_name+".txt", "a")
	...
	appendClassificationReport = open("output/result/WT2nd_KI_"+file_name+".txt", "a")
	...
	data.to_csv("output/wrapperOutput/WT2nd_KI_"+file_name);
